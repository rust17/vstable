...
add:
    - 开发支持环境 (Developer Experience)
        ...
        - [ ] **状态快照校验 (Round-trip Validation)**:
            - **思路**：验证“我认为的状态”是否等于“数据库真实的状态”。
            - **建议**：开发一个简单的验证工具，在执行 `ALTER TABLE` 后，自动调用数据库的 `listColumns` 和 `listIndexes` 接口，将结果与 `SchemaDesigner` 的内存状态进行深度对比（Deep Equal）。
            - **反馈增强**：如果对比失败，工具输出一个 `diff` 报告。这能帮我发现诸如“MySQL 自动修改了字段长度”或“默认值引号处理不一致”等隐蔽问题。
    - 数据表格标签
        - [ ] 鼠标放在上面应该展示表名
