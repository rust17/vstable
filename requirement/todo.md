...
edit:
    - 侧边栏
        - [x] 可以通过拖拽调整宽度
        - [x] 将侧边栏元素与数据表格之间的分割线边距设为 0
        - [x] 将 Schema 下拉选择器替换为与数据库下拉相同的组件，保持交互和样式一致
    - 窗口管理
        - [x] 窗口名称改成当前连接的库
    - 表结构新建页
        - [x] 调整顶部边距，避免与"双击标签栏空白区域最 大化"功能触发区重叠
        - [x] 将"新建页"改为 Tab 形式展示，与"数据表格" 、"Query"等 Tab 并列，移除返回按钮
    - 表结构编辑页
        - [x] 表结构的 name 列宽度太小，调大表结构 name 列的宽度
        - [x] 添加索引时，columns 从 input 改成 checkbox 并允许多选
        - [x] 移除返回按钮
    - 数据表格
        - [x] filter 栏独占一行，宽度 100%
        - [x] 取消 Apply 按钮，回车直接应用 filter
        - [x] 取消 Query 按钮，保持Cmd+T 打开新的 Query Tab 即可
        - 数据表格右键菜单
            - [x] 取消 + Add Row 按钮，通过数据表格右键 菜单实现添加行功能
            - [x] 统一右键菜单：选中行时显示删除（添加置灰），选中空白处时显示添加（删除置灰）
    - 支持选中复制
        - [x] 数据表格：列名
        - [x] 添加表/编辑表：Columns、Indexes、Name 等字段标签

---

fix:
    - 表结构新建页
        - [x] 每一行的宽度与编辑页每一行的宽度不一致

---

add:
    - 选中交互：
        - [x] 连续多选：按住 Shift 点击首尾行，选中区间 内的所有记录
        - [x] 离散多选：按住 Cmd（Mac）/ Ctrl（Windows）点击目标行，逐个增删选中项
        - [x] 框选：按住鼠标左键拖拽，框选选中的记录
    - 数据表格
        - [x] 表允许选中多行记录一起删除
    - 数据库列表
        - [x] 允许选中多个库一起删除
...
add:
    ...
    - 数据表格
        - [x] 通过点击列名快速：倒序、正序、取消排序，支持多个字段一起排序
...
fix:
    - [x] 建表语句太长时没有滚动条，看不到下方的信息
    - [x] 建表弹窗 ctrl+a 会选择页面的数据(应该选中建表语句)
    - [x] ctrl+p 打开的表，点 filter 栏下拉框没有数据
    - [x] query 页面添加下拉滚动，防止看不到最底下的数据
...
add:
    - [ ] 表结构变更场景
        新增字段
        修改字段：
            更改数据类型（如将VARCHAR(50)改为VARCHAR(100)）
            重命名字段（如将用户名称改为username）
            调整默认值或非空约束
        删除字段
        调整列的顺序（MySQL）
        修改表名
    - [ ] 约束变更
        主键约束：添加、删除或更改主键
        外键约束：建立或解除表之间的关联关系，或修改级联删除/更新规则
        唯一约束：防止某列出现重复值
        检查约束：添加或修改字段值的范围限制（如年龄必须大于0）
        默认值约束：设置或修改字段的默认值
    - [ ] 索引变更
        创建索引
        删除索引
