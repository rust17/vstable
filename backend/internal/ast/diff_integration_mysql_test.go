...
func TestMysqlSyncIntegration(t *testing.T) {
	...
	if dsn == "" {
		dsn = "root:password@tcp(localhost:3307)/vstable_test?charset=utf8mb4&parseTime=True&loc=Local"
	}
	...
	cleanup := []string{
		...
		fmt.Sprintf("DROP TABLE IF EXISTS %s;", refTableName),
	...
	// 4. ALTER TABLE: Add Column, Change Column, Rename Column, Drop Column, FK, Check, Index, Config
	...
	verifyColSql := fmt.Sprintf("SELECT COLUMN_NAME, IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '%s' AND TABLE_SCHEMA = '%s';", tableName, schema)
	...
	if err != nil {
		...
		for _, row := range res.Rows {
			...
			for k, v := range row {
				...
				if lowerK == "COLUMN_NAME" || lowerK == "column_name" {
					colName = val
				}
				if lowerK == "IS_NULLABLE" || lowerK == "is_nullable" {
					isNullable = val
				}
			}
			if colName != "" {
	...
	insertSql := fmt.Sprintf("INSERT INTO %s (id, user_name, status, age) VALUES (1, 'test', 'active', 25);", tableName)
	if _, err := driver.Query(ctx, insertSql, nil); err != nil {
		t.Errorf("[%s] Valid insert failed: %v", dialect, err)
	}

	invalidInsertSql := fmt.Sprintf("INSERT INTO %s (id, user_name, status, age) VALUES (2, 'fail', 'active', -1);", tableName)
	...
	viewReq := DiffRequest{
		...
		Views: []ViewDefinition{
			{Name: viewName, Definition: fmt.Sprintf("SELECT id, user_name FROM %s", tableName)},
		},
	}
	...
	verifyViewSql := fmt.Sprintf("SELECT * FROM %s WHERE id = 1;", viewName)
	...
	trigReq := DiffRequest{
		...
		Triggers: []TriggerDefinition{
			{Name: "trig_test", Timing: "BEFORE", Event: "INSERT", TableName: tableName, Definition: "BEGIN SET @x = 1; END"},
		},
	}
	...
	routReq := DiffRequest{
		...
		Routines: []RoutineDefinition{
			{Name: "test_proc", Type: "PROCEDURE", Definition: "CREATE PROCEDURE test_proc() BEGIN SELECT 1; END"},
		},
	}
