...
export const generateAlterTableSql = (
  ...
  if (pkChanged) {
    ...
    if (oldPkConstraint) {
      sqls.push(`ALTER TABLE ${fullTableName} DROP CONSTRAINT "${oldPkConstraint}";`)
    } else {
      // Fallback: try to drop by standard naming if we don't have the name (risky but better than nothing)
      // Actually, it's better to just skip if we don't know the name, or assume tablename_pkey
      // sqls.push(`ALTER TABLE ${fullTableName} DROP CONSTRAINT IF EXISTS "${tableName}_pkey";`)
    }
  }
  ...
  columns.forEach(col => {
    ...
    if (!col._original) {
      ...
      if (col.comment) {
        sqls.push(`COMMENT ON COLUMN ${fullTableName}."${col.name}" IS '${col.comment.replace(/'/g, "''")}';`)
      }
    } else {
      // Modified Column
      ...
      // Identity
      if (col.isIdentity !== original.isIdentity) {
        if (col.isIdentity) {
          sqls.push(`ALTER TABLE ${fullTableName} ALTER COLUMN "${col.name}" ADD GENERATED BY DEFAULT AS IDENTITY;`)
        } else {
          sqls.push(`ALTER TABLE ${fullTableName} ALTER COLUMN "${col.name}" DROP IDENTITY IF EXISTS;`)
        }
      }

      // Comment
      if (col.comment !== original.comment) {
        sqls.push(`COMMENT ON COLUMN ${fullTableName}."${col.name}" IS ${col.comment ? `'${col.comment.replace(/'/g, "''")}'` : 'NULL'};`)
      }
    }
  })
  ...
  // 5. Add New PK if changed
  if (pkChanged) {
    const newPkCols = columns.filter(col => col.isPrimaryKey).map(col => `"${col.name}"`)
    if (newPkCols.length > 0) {
      sqls.push(`ALTER TABLE ${fullTableName} ADD PRIMARY KEY (${newPkCols.join(', ')});`)
    }
  }
