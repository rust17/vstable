    ...
    <div className="flex flex-col h-full overflow-hidden bg-white">
      ...
      <div className="shrink-0 h-11 flex items-center px-2 border-b border-gray-200 bg-[#f8f9fa]">
        <div className="relative flex-1">
          ...
          {dbListOpen && (
            <>
              <div className="fixed inset-0 z-10" onClick={() => { setDbListOpen(false); setSelectedDbs(new Set()); }} />
                ...
                {selectedDbs.size > 1 && (
                    <div className="p-2 border-t border-red-100 bg-red-50/50">
                        <button 
                            onClick={(e) => {
                                e.stopPropagation()
                                if (window.confirm(`Are you sure you want to delete ${selectedDbs.size} databases?`)) {
          ...
          <div className="flex items-center justify-between mb-3 pb-1 border-b border-gray-100 shrink-0 px-1">
            ...
            <div className="flex items-center gap-1">
              {selectedTables.size > 1 && (
                <button 
                  onClick={() => {
                      if (window.confirm(`Are you sure you want to delete ${selectedTables.size} tables?`)) {
                        ...
                        onDeleteTable(currentSchema, Array.from(selectedTables).map(k => k.split('.')[1]))
