  ...
  return (
    ...
    >
      <button
        ...
        className="w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-primary-50 flex items-center gap-2"
      >
      ...
      <button
        ...
        className="w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-primary-50 flex items-center gap-2 border-b border-gray-100 mb-1 pb-2"
      >
      ...
      >
        {selectedColumns.length > 0 ? (
          selectedColumns.map((c) => (
            <span
              key={c}
              className="px-1.5 py-0.5 bg-purple-100 text-purple-700 rounded text-[10px] font-medium flex items-center gap-1"
            >
              {c}
              <X
                size={10}
...
export const StructureView: React.FC<StructureViewProps> = ({
  ...
  const [newSchema, setNewSchema] = useState(
    initialSchema || (capabilities?.supportsSchemas ? 'public' : config.database)
  );
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; colId: string } | null>(
    null
  );
  ...
  const fetchStructure = useCallback(async () => {
    ...
    setDeletedIndexes([]);
    try {
      const colSql = buildQuery('listColumns', {
        db: config.database,
        schema: initialSchema,
        table: initialTableName,
      });
      const colRes = await apiClient.query(connectionId, colSql);
      ...
      const idxSql = buildQuery('listIndexes', {
        db: config.database,
        schema: initialSchema,
        table: initialTableName,
      });
      const idxRes = await apiClient.query(connectionId, idxSql);
  ...
  const handlePreviewSql = async (type: 'diff' | 'create' = 'diff') => {
    ...
    try {
      ...
      if (type === 'create' || mode === 'create') {
        ...
        sqls = await apiClient.generateCreateSql({
        ...
        sqls = await apiClient.generateAlterSql({
          dialect: capabilities.dialect,
          schema: initialSchema,
          tableName: initialTableName,
      ...
      <div className="flex items-center justify-between px-6 h-[94px] bg-white border-b border-gray-200">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {availableSchemas.length > 0 ? (
                    ...
                    <option value={capabilities?.supportsSchemas ? 'public' : config.database}>
                      {capabilities?.supportsSchemas ? 'public' : config.database}
                    </option>
                  )}
                </select>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={() => handlePreviewSql('diff')}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 shadow-sm transition-all"
          >
        ...
        <div className="border-b border-gray-100">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-[#f8f9fa] select-text">
            ...
            <button
              ...
              className="text-xs flex items-center gap-1 px-3 py-1.5 bg-primary-50 text-primary-600 rounded hover:bg-primary-100 font-medium transition-colors"
            >
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-white text-gray-500 font-medium border-b border-gray-100 select-text">
                <tr>
                  ...
                  <th className="px-6 py-3 min-w-[200px] text-[11px] uppercase tracking-wider">
                    Name
                  </th>
                  ...
                  <th className="px-6 py-3 w-20 text-center text-[11px] uppercase tracking-wider">
                    Auto
                  </th>
                  <th className="px-6 py-3 w-20 text-center text-[11px] uppercase tracking-wider">
                    ID
                  </th>
                  <th className="px-6 py-3 w-24 text-center text-[11px] uppercase tracking-wider">
                    Nullable
                  </th>
                  <th className="px-6 py-3 w-24 text-center text-[11px] uppercase tracking-wider">
                    Primary
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-3">
                        <div
                          className={`text-gray-400 text-sm font-mono flex items-center gap-2 ${isDraggable ? 'cursor-move' : ''}`}
                        >
                          {isDraggable && (
                          ...
                          {hasPrecision && (
                            <>
                              <input
                                ...
                                value={col.precision || ''}
                                onChange={(e) =>
                                  handleColumnChange(col.id, 'precision', e.target.value)
                                }
                              ...
                              {(typeLower.includes('numeric') || typeLower.includes('decimal')) && (
                                <input
                                  ...
                                  value={col.scale || ''}
                                  onChange={(e) =>
                                    handleColumnChange(col.id, 'scale', e.target.value)
                                  }
                                  className="w-10 bg-gray-100/50 border border-transparent rounded px-1 py-0.5 text-[10px] focus:bg-white focus:border-primary-300 outline-none"
                                  title="Scale"
                                />
                      ...
                      <td className="px-6 py-3 text-center">
                        {canAutoInc && (
                          <input
                            ...
                            onChange={(e) =>
                              handleColumnChange(col.id, 'isAutoIncrement', e.target.checked)
                            }
                      ...
                      <td className="px-6 py-3 text-center">
                        {canAutoInc && (
                          <input
                            ...
                            onChange={(e) =>
                              handleColumnChange(col.id, 'isIdentity', e.target.checked)
                            }
                            className="rounded border-gray-300 text-primary-600 focus:ring-primary-500 cursor-pointer"
                      ...
                      <td className="px-6 py-3 text-center">
                        <div
                          onClick={() =>
                            handleColumnChange(col.id, 'isPrimaryKey', !col.isPrimaryKey)
                          }
                          ...
                          {col.defaultValue !== null ? (
                            <div className="flex flex-1 items-center bg-gray-100/50 border border-gray-200 rounded px-1 transition-all focus-within:border-primary-300 focus-within:bg-white">
                              <input
                                ...
                                onChange={(e) =>
                                  handleColumnChange(col.id, 'defaultValue', e.target.value)
                                }
                                placeholder="Value..."
        ...
        <div className="bg-white border-b border-gray-100">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-[#f8f9fa] select-text">
            ...
            <button
              ...
              className="text-xs flex items-center gap-1 px-3 py-1.5 bg-purple-50 text-purple-600 rounded hover:bg-purple-100 font-medium transition-colors"
            >
        ...
        createPortal(
          ...
          >
            ...
            >
              <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-t-xl">
                <h3 className="font-semibold text-gray-700 flex items-center gap-2">
                  <Database size={16} className="text-primary-500" /> {previewTitle}
                </h3>
                <button
                  onClick={() => setSqlPreview(null)}
                  className="p-1 hover:bg-gray-100 rounded-full transition-colors"
              ...
              <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 bg-white rounded-b-xl">
                ...
                {showExecuteInPreview && sqlPreview !== '-- No changes detected --' && (
                  <button
                    data-testid="btn-execute-sql"
                    onClick={executeChanges}
                    className="px-6 py-2 text-sm font-medium bg-primary-600 text-white rounded-lg hover:bg-primary-700 shadow-sm transition-colors flex items-center gap-2"
                  >
