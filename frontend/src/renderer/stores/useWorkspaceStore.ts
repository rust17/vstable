  ...
  return createStore<WorkspaceState>((set, get) => ({
    ...
    setActiveTabId: (id) => set((state) => {
      if (id && id !== state.activeTabId && !state.showTabSwitcher) {
        const filtered = state.mruTabIds.filter(mruId => mruId !== id && state.tabs.some(t => t.id === mruId))
    ...
    openTable: (schema, name) => {
      const state = get()
      const existingTab = state.tabs.find(t => t.type === 'table' && t.schema === schema && t.name === name)
      if (existingTab) {
    ...
    openStructure: (schema?, name?, mode = 'edit') => {
      ...
      if (mode === 'edit' && schema && name) {
          const existingTab = state.tabs.find(t => t.type === 'structure' && t.initialSchema === schema && t.initialTableName === name)
          if (existingTab) {
              get().setActiveTabId(existingTab.id)
              return existingTab
          }
    ...
    closeToRight: (tabId) => {
      set(state => {
        ...
        const newActive = remaining.some(t => t.id === state.activeTabId) ? state.activeTabId : tabId
        return {
          tabs: remaining,
