...
export const createSessionStore = (id: string, onUpdateTitle: (title: string) => void) => {
  return createStore<SessionState>((set, get) => ({
      ...
      try {
        ...
        if (result.success) {
          set({
            isConnected: true,
            config: newConfig,
            capabilities: result.capabilities || getInitialCapabilities(newConfig.dialect || 'postgres')
          })
    ...
    buildQuery: (templateKey: keyof Capabilities['queryTemplates'], vars: Record<string, string>) => {
