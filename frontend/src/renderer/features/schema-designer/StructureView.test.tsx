...
describe('StructureView Component', () => {
  ...
  it('handles identity columns and comments', async () => {
    ;(window as any).api.query.mockImplementation((id, sql) => {
      if (sql.includes('information_schema.columns')) {
        return Promise.resolve({
          success: true,
          rows: [
            { column_name: 'id', data_type: 'integer', is_nullable: 'NO', is_identity: 'YES', column_comment: 'Primary ID' }
          ]
        })
      }
      return Promise.resolve({ success: true, rows: [] })
    ...
    const identityCheckbox = screen.getByTitle('GENERATED BY DEFAULT AS IDENTITY') as HTMLInputElement
